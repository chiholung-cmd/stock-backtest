#!/usr/bin/env python3
"""
Stock Backtest Engine v2
Supports: MA Crossover, RSI, MACD, Bollinger Bands, KD, Price Action, Predator MF (with Bear Market Cash Accumulation)
...
def simulate_trades_with_dca(df: pd.DataFrame, signals: pd.Series, cash_flows: pd.Series, initial_capital: float = 10000.0):
    """Simulate trades with monthly DCA cash flows. Returns equity curve and trade list."""
    ...
    for date, row in df.iterrows():
        ...
        cash_flow = cash_flows.get(date, 0.0)

        # Add monthly DCA cash
        if cash_flow > 0:
            equity += cash_flow
            trades.append({
                "date": str(date.date()),
...
# ─── Strategy: Predator MF (TQQQ Optimized with Bear Market Cash Accumulation) ──────────────────────────────────
...
def strategy_predator_mf(df: pd.DataFrame, params: dict):
    ...
    monthly_dca = float(params.get("monthlyDCA", 3000))
    bear_market_dca = float(params.get("bearMarketDCA", 6000))
    ...
    signals = pd.Series(0, index=df.index)
    cash_flows = pd.Series(0.0, index=df.index)
    position = 0
    entry_price = 0.0
    ...
    for i in range(len(df)):
        ...
        rsi = df["rsi"].iloc[i]
        is_bear = price < sma_slow
        
        # Monthly DCA logic
        current_month = date.to_period('M')
        if last_dca_month is None or current_month != last_dca_month:
            if is_bear:
                cash_flows.iloc[i] = bear_market_dca  # 6,000 HKD in bear market
            else:
                cash_flows.iloc[i] = monthly_dca  # 3,000 HKD in bull market
...
def run_backtest(ticker: str, strategy: str, params: dict, start_date: str, end_date: str) -> dict:
    ...
    if strategy == "predator_mf":
        df_with_indicators, signals, cash_flows = strategy_fn(df, params)
        equity_curve, equity_series, trades = simulate_trades_with_dca(df_with_indicators, signals, cash_flows)
    else:
